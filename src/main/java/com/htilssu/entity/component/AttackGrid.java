...
public class AttackGrid extends BattleGrid {
    ...
    private void clearHighlight() {
        for (Component comp : self.getComponents()) {
            if (comp instanceof JPanel) {
                ...
                if (!Color.WHITE.equals(cell.getBackground()) && !Color.RED.equals(
                        cell.getBackground())) {
    ...
    //Khi người chơi nhấp vào một ô, nó sẽ kiểm tra xem lưới tấn công có đang hoạt động hay không
    // (isAttackGridListener),
                            ...
                            if (isSunk) {
                                ...
                                if (lost) {
                                    ...
                                    JOptionPane.showMessageDialog(null,
                                            "You(player 1) WON! Congratulations!\nClick OK will " +
                                                    "Exit the game");
                                    battleShip.player1Turn();
                                }

                                enemyShipSunkPlayer1++;
                                battleShip.getPlayer1().enemyShipSunk.setText(
                                        Integer.toString(enemyShipSunkPlayer1));
                                JOptionPane.showMessageDialog(null,
                                        "Thuyen dich bi danh chim!\nnhan OK chuyen sang man hinh " +
                                                "player2");
                                player.hideScreen();
                                battleShip.getPlayer2().showScreen();
                                String ownShipSunkPlayer2 = Integer.toString(
                                        battleShip.getPlayer2Data().getNumberOfOwnShipSunk());
                        ...
                        if (battleShip.getTakeTurnAttack()) {
                            ...
                            if (isSunk) {
                                ...
                                if (lost) {
                                    battleShip.setState(battleShip.getEndOfTheGame());
                                    JOptionPane.showMessageDialog(null,
                                            "You(player 2) WON! Congratulations!\nClick OK will " +
                                                    "Exit the game");
                                    battleShip.player2turn();
                                }

                                enemyShipSunkPlayer2++;
                                battleShip.getPlayer2().enemyShipSunk.setText(
                                        Integer.toString(enemyShipSunkPlayer2));
                                JOptionPane.showMessageDialog(null,
                                        "Chuc mung! Thuyen da duoc danh chim!\nnhan OK chuyen " +
                                                "sang man hinh player1");
                                player.hideScreen();
                                battleShip.getPlayer1().showScreen();
                                String ownShipSunkPlayer1 = Integer.toString(
                                        battleShip.getPlayer1Data().getNumberOfOwnShipSunk());
            ...
            for (int j = 0; j < 11; j++) {
                ...
                if (temp[i][j] == 1) {
                    ...
                    if (thePanel instanceof GamePanel gamePanel) {
                        gamePanel.setBackgroundImage(
                                AssetUtils.getImage(AssetUtils.ASSET_SHOOT_MISS));
                    }
                }
                //khong trung tau
                if (temp[i][j] == 2) {
